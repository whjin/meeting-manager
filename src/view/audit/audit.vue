...
<script>
import {
  getAuditDataByParams,
  getRoomCountByParams,
  submitAuditResult
} from '@/api/bg-manage/audit/manage';
...
export default {
  ...
  methods: {
    getAuditType (name) {
      if (name === 'default') {
        this.searchCon.data.EQ_odsYyLshjDO_appointmentDate = formatDate(
          this.orderTime
        );
        this.searchCon.data.IN_odsYyLshjDO_appointmentApprover = '0,2';
        this.getAuditingList(this.searchCon);
      } else {
        this.searchSuccessCon.data.EQ_odsYyLshjDO_appointmentDate = formatDate(
          this.orderTime
        );
    ...
    getRoomCountList () {
      let roomInfoParams = {
        data: [
          ...
          '15:00-16:00',
          '16:00-17:00',
          formatDateToStr(this.orderTime)
        ]
      };
