<template>
  <div class="login">
    <div class="login-con">
      <Card title="律师远程会见平台" :bordered="false">
        <div class="form-con">
...
export default {
  ...
  methods: {
    ...
    loginConfirm () {
      this.handleLogin(this.loginInfo).then(res => {
        if (res.data.state.code === 200) {
          if (res.data.data.modifiePwd) {
            this.$Message.error('密码已超过有效期，请尽快修改密码');
          }
          this.getUserInfo().then(res => {
            Cookies.set('lock', false);
            this.$router.push('/meetRegister');
          });
        }
      });
    },
