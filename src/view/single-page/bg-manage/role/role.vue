    ...
    <div>
      ...
      <i-row>
        <i-col class="tableCol"  style="border: 0;">
          ...
          <i-table border
                   ...
                   :columns="columns"
                   :data="roleList"
                   style="height:calc(100% - 16vw);position:absolute;"
                   @on-selection-change="getSelectedRows">
          </i-table>
          <div class="tablePage selectionIcon">
            <i-page style="text-align: right"
              ref="pages" :total="roleListLength" :page-size="pageSize" :page-size-opts="pageSizeOpts" show-total
      ...
      <div class="add-role-dialog">
        <i-modal v-model="showEditModal"
                 @on-cancel="cancelSubmit('selectedRole')"
                 width="740"
                 class="role-info-modal">
...
-->
                    <i-select class="selectionIcon" v-model="selectedRole.roleDesc"  @on-change="changeDataAuth(selectedRole.roleDesc)">
                      <i-option v-for="(item, index) in roleTypeList" :value="item.id" :key="index">{{ item.value }}
                      </i-option>
                    </i-select>
                  </i-form-item>
                </i-col>
                ...
                <i-col span="12">
                  <i-form-item label="数据查询权限" prop="dataAuth" :label-width="120">
                    <i-radio-group v-model="selectedRole.dataAuth" >
                      <i-radio :label="1" disabled>所在部门</i-radio>
                      <i-radio :label="2" disabled>全部</i-radio>
                    </i-radio-group>
                  </i-form-item>
                </i-col>
              </i-row>
              <i-row>
                ...
                <!--<i-col span="12">
                  <i-form-item label="角色类型" :label-width="120" prop="type">
                  <DictionarySelect class="selectionIcon" code="SYS_DICT_ROLE_TYPE"
                                      placeholder="请选择角色类型"&ndash;&gt;
                                     :checkValue.sync="selectedRole.type">
                   </DictionarySelect>
                    <select-box :idx="1" :url="url" :submitClear="submitClear" :modify="selectedRole.typeName" typeCode="SYS_DICT_ROLE_TYPE" @val="getRoleTypeItem" style="margin: 0 0"></select-box>
                  </i-form-item>
                </i-col>-->

                <i-col span="12">
                  <i-form-item label="角色描述">
                    <i-input v-model="selectedRole.info" placeholder="请输入描述"></i-input>
                  </i-form-item>
                </i-col>
              </i-row>
            </i-form>
      ...
      <div class="role-info-dialog">
        <i-modal class="infoDialogClass" v-model="isShowInfoModal" width="700" @on-ok="isShowInfoModal = false">
          ...
          <div class="objInfoModal" style="height:130px;">
            ...
            <i-tooltip :content="selectedRole.info" placement="top-start" v-show="selectedRole.info">
              角色描述： {{selectedRole.info}}
            </i-tooltip>
            <div class="unToolTip" v-show="!selectedRole.info">
              角色描述：{{selectedRole.info}}
            </div>
...
<script>
import { getRoleWithParam, addNewRole, getRoleDetail, searchBoxDataUrl,
  getTreeByRoleId, updateMenuRoleConf, deleteRoles, updateRole } from '@/api/bg-manage/role';
import confirmBox from '_c/confirmBox';
import { validateDataAuth, validateNormal } from '@/libs/verification.js';
/*
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            const text = row.roleDesc === '1' ? '系统管理员' : row.roleDesc === '2' ? '监所民警' : '系统维护员';
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row;
            const text = row.dataAuth === 1 ? '所在部门' : '全部';
            if (text === undefined || text === '') {
              return h('div', text);
            } else {
              return h('Tooltip', {
      ...
      ruleValidate: {
        ...
        dataAuth: [
          { required: true, validator: validateDataAuth, message: '请选择权限', trigger: 'change' }
        ],
        type: [
          { required: true, validator: validateNormal, trigger: 'blur' }
        ]
  ...
  methods: {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          if (this.isAddFunction) {
            ...
            if (this.selectedRole.roleDesc !== '' && this.selectedRole.roleDesc !== undefined) {
    ...
    changeDataAuth (type) {
      if (type === '1') {
        this.selectedRole.roleName = '系统管理员';
        this.selectedRole.dataAuth = 2;
      } else if (type === '3') {
        this.selectedRole.roleName = '系统维护员';
        this.selectedRole.dataAuth = 2;
      } else if (type === '3') {
        this.selectedRole.roleName = '监所民警';
...
<style lang="less">
  ...
  .role-info-modal /deep/ .ivu-radio-disabled /deep/ .ivu-radio-inner:after {
