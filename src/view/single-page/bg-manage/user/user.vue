<template>
  <div>
    ...
    <i-row>
      <i-split v-model="split" min="200" max="650" >
        ...
        <i-col slot="right" class="tableCol">
          ...
          <!-- <span class="searchItemText">岗位</span>
          <i-select class="selectionIcon" ref="selectModel" clearable v-model="searchData.roleName">
            <i-option v-for="(item,index) in roleListData" :value="item.id" :key="index">
              {{item.roleName}}
            </i-option>
          </i-select> -->
          <i-button type="primary" style="display: inline-block;margin-left: 69px;" @click="searchParams()">查询</i-button>
    ...
    <!--重置密码确认框-->
    <confirm-box :confirmModel="resetPassModel"
                 :confirmActive="resetPassActive"
                 @passModel="updateResetPassModel($event)"
                 @confirmValue="resetPassValue($event)">
    </confirm-box>
    <!--查看用户信息的模态框-->
    <i-modal width="600" v-model="isShowInfoModal" @on-ok="isShowInfoModal = false">
      ...
      <div class="objInfoModal" style="height:290px;">
        ...
        <!--<i-tooltip :content="selectedUser.role" placement="top-start" v-show="selectedUser.role">
          岗位：{{selectedUser.role}}
        </i-tooltip>
        <div class="unToolTip" v-show="!selectedUser.role">
          岗位：{{selectedUser.role}}
        </div>-->
      </div>
      <div slot="footer">
    ...
    <i-modal v-model="showEditModal" width="800" @on-cancel="cancelSubmit('selectedUser')">
      ...
      <div class="userInfo">
        <i-form ref="selectedUser" :model="selectedUser" :label-width="120" :rules="ruleValidate">
          ...
          <!--<i-form-item label="岗位" prop="post">
            <i-select class="selectionIcon" ref="selectModel3" clearable v-model="selectedUser.post" placeholder="请选择岗位">
              <i-option v-for="(item,index) in roleListData" :value="item.id" :key="index">{{item.roleName}}
              </i-option>
            </i-select>
          </i-form-item>-->
          <i-form-item label="所属部门" prop="organizeId">
            <i-select class="selectionIcon" ref="selectModel1" clearable v-model="selectedUser.organizeId" placeholder="请选择所属部门">
          ...
          <i-form-item label="工号" prop="userCode">
            ...
            <i-input v-model="selectedUser.userCode" placeholder="请输入工号" v-show="false"></i-input>
          </i-form-item>
          <i-form-item label="密码" prop="password" :style="{'display': isAddFunction?'inline-block':'none'}">
            <i-input class="password-input" v-model="selectedUser.password" placeholder="请输入密码"></i-input>
            <i-input class="password-input" v-model="selectedUser.password" placeholder="请输入密码" v-show="false"></i-input>
          </i-form-item>
          <i-form-item label="确认密码" prop="password2" :style="{'display': isAddFunction?'inline-block':'none'}">
            <i-input class="password-input" v-model="selectedUser.password2" placeholder="请确认密码"></i-input>
            <i-input class="password-input" v-model="selectedUser.password2" placeholder="请确认密码" v-show="false"></i-input>
          </i-form-item>
          <i-form-item label="性别" prop="gender">
            <i-radio-group v-model="selectedUser.gender" style="width:10.417vw;">
              <!--用:prop传递数据格式为 数字、布尔值或函数时，必须带:(兼容String除外，具体看组件文档)-->
    ...
    <div class="user-role-tra-dialog">
      <i-modal v-model="showTransferDialog" width="600px;">
        ...
        <div style="height:250px;">
          <i-transfer :data="userUnHadRole"
                      :target-keys="userHadRole"
      ...
      if (this.isAddFunction) {
        if (value === '' || value === undefined) {
          ...
          if (reg) {
            ...
            return callback(new Error('密码不能含有中文字符'));
          }
