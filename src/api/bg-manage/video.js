...
const initParams = params => {
  ...
  if (params.port != null && params.port !== undefined) {
    if (
      !/^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5])$/.test(params.port)
    ) {
      console.log("端口填写有误！");
      return;
    } else {
  ...
  if (params.playMode != null && params.playMode !== undefined) {
    paramsOptions.playMode = parseInt(params.playMode);
  }
  if (params.layout != null && params.layout !== undefined) {
    paramsOptions.layout = replaceParam(params.layout);
  }
  if (params.showToolbar != null && params.showToolbar !== undefined) {
    paramsOptions.showToolbar = parseInt(params.showToolbar);
  }
  if (params.showSmart != null && params.showSmart !== undefined) {
    paramsOptions.showSmart = parseInt(params.showSmart);
  }
  if (params.buttonIDs != null && params.buttonIDs !== undefined) {
    paramsOptions.buttonIDs = replaceParam(params.buttonIDs);
  }
  if (params.snapDir != null && params.snapDir !== undefined) {
    paramsOptions.snapDir = replaceParam(params.snapDir);
  }
  if (params.videoDir != null && params.videoDir !== undefined) {
    paramsOptions.videoDir = replaceParam(params.videoDir);
  }
  if (params.encryptedFields != null && params.encryptedFields !== undefined) {
    paramsOptions.encryptedFields = replaceParam(params.encryptedFields);
  }
  console.log(JSON.stringify(paramsOptions));
